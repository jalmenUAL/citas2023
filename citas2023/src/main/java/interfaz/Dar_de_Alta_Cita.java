	...
	public void Dar_de_Alta() {
		if (_poner_Cliente._cliente) {
			if (_poner_Asunto._asunto) {
				adm.Crear_Cita_Asunto_Cliente_Nuevos(_poner_Fecha.getFechaCita().getValue(),
						_poner_Asunto._nuevo_Asunto.getNuevoAsunto().getValue(),
						_poner_Cliente._nuevo_Cliente.getNuevoCliente().getValue(),
						_poner_Cliente._nuevo_Cliente.getDireccion().getValue(),
						_poner_Cliente._nuevo_Cliente.getTelefono().getValue(),
						_poner_Cliente._nuevo_Cliente.getCorreo().getValue());
				...
				Enviar_Correo(_poner_Cliente._nuevo_Cliente.getCorreo().getValue(),
						_poner_Fecha.getFechaCita().getValue(),
						_poner_Asunto._nuevo_Asunto.getNuevoAsunto().getValue());

			} else {
				if (_poner_Asunto.getSeleccionAsunto().getValue() == null) {
					Notification.show("Por favor, selecciona un asunto");
				} else {
					if (_poner_Fecha.getFechaCita().getValue() == null) {
						Notification.show("Por favor, selecciona una fecha");
					} else {
						Asunto asunto = _poner_Asunto.getSeleccionAsunto().getValue();
						adm.Crear_Cita_Nuevo_Cliente(_poner_Fecha.getFechaCita().getValue(),
								_poner_Cliente._nuevo_Cliente.getNuevoCliente().getValue(),
								_poner_Cliente._nuevo_Cliente.getDireccion().getValue(),
								_poner_Cliente._nuevo_Cliente.getTelefono().getValue(), asunto.getORMID(),
								_poner_Cliente._nuevo_Cliente.getCorreo().getValue());
						_poner_Cliente._cliente = false;
						_poner_Cliente.getVaadinVerticalLayout().as(VerticalLayout.class)
								.remove(_poner_Cliente._nuevo_Cliente);
						Enviar_Correo(_poner_Cliente._nuevo_Cliente.getCorreo().getValue(),
								_poner_Fecha.getFechaCita().getValue(),
								_poner_Asunto.getSeleccionAsunto().getValue().getNombre());
			...
			if (_poner_Asunto._asunto) {
				if (_poner_Cliente.getSeleccionCliente().getValue() == null) {
					Notification.show("Por favor, selecciona un cliente");
				}
					...
					if (_poner_Fecha.getFechaCita().getValue() == null) {
						...
						adm.Crear_Cita_Nuevo_Asunto(_poner_Fecha.getFechaCita().getValue(),
								_poner_Asunto._nuevo_Asunto.getNuevoAsunto().getValue(), cliente.getORMID());
						_poner_Asunto._asunto = false;
						_poner_Asunto.getVaadinVerticalLayout().as(VerticalLayout.class)
								.remove(_poner_Asunto._nuevo_Asunto);
						Enviar_Correo(_poner_Cliente._nuevo_Cliente.getCorreo().getValue(),
								_poner_Fecha.getFechaCita().getValue(),
								_poner_Asunto._nuevo_Asunto.getNuevoAsunto().getValue());
				...
				if (_poner_Cliente.getSeleccionCliente().getValue() == null) {
					Notification.show("Por favor, selecciona un cliente");
				} else if (_poner_Asunto.getSeleccionAsunto().getValue() == null) {
					Notification.show("Por favor, selecciona un asunto");
				} else if (_poner_Fecha.getFechaCita().getValue() == null) {
					Notification.show("Por favor, selecciona una fecha");
				} else {
					Asunto asunto = _poner_Asunto.getSeleccionAsunto().getValue();
					Cliente cliente = _poner_Cliente.getSeleccionCliente().getValue();
